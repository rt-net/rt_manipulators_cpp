...
void set_right_arm_joint_positions(std::vector<manipulators_link::Link> & links,
...
void set_left_arm_joint_positions(std::vector<manipulators_link::Link> & links,
                                  std::vector<double> positions) {
...
void set_torso_joint_positions(std::vector<manipulators_link::Link> & links,
                               std::vector<double> positions) {
...
int main() {
  std::cout << "現在姿勢をもとに重力補償トルクを計算し、"
            << "Sciurus17のサーボモータに入力するサンプルです" << std::endl;
  ...
  std::vector<kinematics_utils::link_id_t> target_id_list{target_id_right, target_id_left};
  // 関節可動範囲の設定
  for (const auto & target_id : target_id_list) {
    for (auto link_id : kinematics_utils::find_route(links, target_id)) {
