...
const double TO_ACCELERATION_TO_RAD_PER_MM =
    TO_ACCELERATION_REV_PER_MM * 2.0 * M_PI;
const double TO_ACCELERATION_TO_RAD_PER_SS =
    TO_ACCELERATION_TO_RAD_PER_MM / 3600.0;
const double DXL_ACCELERATION_FROM_RAD_PER_SS =
    1.0 / TO_ACCELERATION_TO_RAD_PER_SS;
...
bool Hardware::load_config_file(const std::string& config_yaml) {
  ...
  for (auto group : joint_groups_) {
    ...
    for (auto joint_name : group.second->joint_names()) {
      ...
      std::cout << ", mode:"
                << std::to_string(all_joints_.at(joint_name)->operating_mode());
      std::cout << std::endl;
    }
  }
...
bool Hardware::connect(const int baudrate) {
  if (!port_handler_->setBaudRate(baudrate)) {
    std::cerr << "Unable to set baudrate: " << std::to_string(baudrate)
              << std::endl;
    return false;
  }
  if (!port_handler_->openPort()) {
    std::cerr << "Unable to open port: " << port_handler_->getPortName()
              << std::endl;
    return false;
  }
...
bool Hardware::torque_on(const std::string& group_name) {
  ...
  if (!sync_read(group_name)) {
    std::cerr << group_name << "グループのトルクON後のsync_readに失敗しました."
              << std::endl;
    return false;
  }
...
bool Hardware::sync_read(const std::string& group_name) {
  ...
  if (!sync_read_groups_[group_name]) {
    std::cerr << group_name << "にはsync_readが設定されていません."
              << std::endl;
    return false;
  }
  ...
  if (joint_groups_[group_name]->sync_read_position_enabled()) {
    for (auto joint_name : joint_groups_[group_name]->joint_names()) {
      ...
      if (!sync_read_groups_[group_name]->isAvailable(
              id, address_indirect_present_position_[group_name],
              LEN_PRESENT_POSITION)) {
        std::cerr << std::to_string(id) << "のpresent_positionを取得できません."
                  << std::endl;
        retval = false;
      } else {
        int32_t data = sync_read_groups_[group_name]->getData(
            ...
            LEN_PRESENT_POSITION);
        all_joints_.at(joint_name)
            ->set_present_position(dxl_pos_to_radian(data));
      }
    }
  }
...
bool Hardware::sync_write(const std::string& group_name) {
  ...
  if (!sync_write_groups_[group_name]) {
    std::cerr << group_name << "にはsync_writeが設定されていません."
              << std::endl;
    return false;
  }
  ...
  for (auto joint_name : joint_groups_[group_name]->joint_names()) {
    ...
    if (joint_groups_[group_name]->sync_write_position_enabled()) {
      uint32_t goal_position =
          radian_to_dxl_pos(all_joints_.at(joint_name)->get_goal_position());
...
bool Hardware::start_thread(const std::vector<std::string>& group_names,
  ...
  thread_enable_ = true;
  read_write_thread_ = std::make_shared<std::thread>(
      &Hardware::read_write_thread, this, group_names, update_cycle_ms);
...
bool Hardware::get_position(const uint8_t id, double& position) {
  if (!all_joints_contain_id(id)) {
    std::cerr << "ID:" << std::to_string(id) << "のジョイントは存在しません."
              << std::endl;
    return false;
  }
...
bool Hardware::get_positions(const std::string& group_name,
...
bool Hardware::set_position(const std::string& joint_name,
...
bool Hardware::set_positions(const std::string& group_name,
                             std::vector<double>& positions) {
  if (!joint_groups_contain(group_name)) {
    std::cerr << group_name << "はjoint_groupsに存在しません." << std::endl;
    return false;
  ...
  if (joint_groups_[group_name]->joint_names().size() != positions.size()) {
    ...
    std::cerr << "がジョイント数:"
              << joint_groups_[group_name]->joint_names().size();
...
bool Hardware::write_max_acceleration_to_group(const std::string& group_name,
  ...
  // 指定されたグループ内のサーボモータの最大動作加速度（radian /
  // s^2）を設定する
  if (!joint_groups_contain(group_name)) {
    std::cerr << group_name << "はjoint_groupsに存在しません." << std::endl;
    return false;
  ...
  if (!write_double_word_data_to_group(group_name, ADDR_PROFILE_ACCELERATION,
                                       dxl_acceleration)) {
    std::cerr << group_name
              << "グループのProfile Accelerationの書き込みに失敗しました."
              << std::endl;
    return false;
  }
...
bool Hardware::write_max_velocity_to_group(const std::string& group_name,
  ...
  if (!write_double_word_data_to_group(group_name, ADDR_PROFILE_VELOCITY,
                                       dxl_velocity)) {
    std::cerr << group_name
              << "グループのProfile Velocityの書き込みに失敗しました."
              << std::endl;
    return false;
  }
...
bool Hardware::write_position_pid_gain_to_group(const std::string& group_name,
                                                const uint16_t p,
                                                const uint16_t i,
                                                const uint16_t d) {
  ...
  if (!write_word_data_to_group(group_name, ADDR_POSITION_P_GAIN, p)) {
    std::cerr << group_name
              << "グループのPosition P Gainの書き込みに失敗しました."
              << std::endl;
    return false;
  }
  ...
  if (!write_word_data_to_group(group_name, ADDR_POSITION_I_GAIN, i)) {
    std::cerr << group_name
              << "グループのPosition I Gainの書き込みに失敗しました."
              << std::endl;
    return false;
  }
  ...
  if (!write_word_data_to_group(group_name, ADDR_POSITION_D_GAIN, d)) {
    std::cerr << group_name
              << "グループのPosition D Gainの書き込みに失敗しました."
              << std::endl;
    return false;
  }
...
bool Hardware::parse_config_file(const std::string& config_yaml) {
  ...
  if (!fs.is_open()) {
    std::cerr << "コンフィグファイル:" << config_yaml << "が存在しません."
              << std::endl;
    return false;
  }
  ...
  for (auto config_joint_group : config["joint_groups"]) {
    ...
    if (!config["joint_groups"][group_name]["joints"]) {
      std::cerr << group_name << "グループに'joints'が設定されていせん。"
                << std::endl;
      return false;
    }
    ...
    for (auto config_joint : config["joint_groups"][group_name]["joints"]) {
      ...
      if (all_joints_contain(joint_name)) {
        std::cerr << joint_name << "ジョイントが2つ以上存在します."
                  << std::endl;
        return false;
      }

      if (!config[joint_name]) {
        std::cerr << joint_name << "ジョイントの設定が存在しません."
                  << std::endl;
        return false;
      }
      ...
      if (!config[joint_name]["id"] || !config[joint_name]["operating_mode"]) {
        std::cerr << joint_name
                  << "にidまたはoperating_modeが設定されていません."
                  << std::endl;
        return false;
      }
      ...
      all_joints_ref_from_id_.emplace(
          joint_id, joint_ptr);  // IDからもJointにアクセスできる
    }
    ...
    if (config["joint_groups"][group_name]["sync_read"]) {
      sync_read_targets = config["joint_groups"][group_name]["sync_read"]
                              .as<std::vector<std::string>>();
    }
    if (config["joint_groups"][group_name]["sync_write"]) {
      sync_write_targets = config["joint_groups"][group_name]["sync_write"]
                               .as<std::vector<std::string>>();
    }

    auto joint_group_ptr = std::make_shared<joint::JointGroup>(
        joint_names, sync_read_targets, sync_write_targets);
    joint_groups_.emplace(group_name, joint_group_ptr);

    if (!create_sync_read_group(group_name)) {
      std::cerr << group_name << "のsync readグループを作成できません."
                << std::endl;
      return false;
    }
    if (!create_sync_write_group(group_name)) {
      std::cerr << group_name << "のsync writeグループを作成できません."
                << std::endl;
      return false;
    }
  }
...
bool Hardware::create_sync_read_group(const std::string& group_name) {
  ...
  if (joint_groups_[group_name]->sync_read_position_enabled()) {
    if (!set_indirect_address(group_name, indirect_address,
                              ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION)) {
      std::cerr << group_name
                << "グループのpresent_"
                   "positionをインダイレクトアドレスにセットできません."
                << std::endl;
      return false;
    }
    address_indirect_present_position_[group_name] =
        ADDR_INDIRECT_DATA_1 + total_length;
  ...
  sync_read_groups_[group_name] = std::make_shared<dynamixel::GroupSyncRead>(
      port_handler_.get(), packet_handler_.get(), ADDR_INDIRECT_DATA_1,
      total_length);

  for (auto joint_name : joint_groups_[group_name]->joint_names()) {
    auto id = all_joints_.at(joint_name)->id();
    if (!sync_read_groups_[group_name]->addParam(id)) {
      std::cerr << group_name << ":" << joint_name
                << "のgroupSyncRead.addParam に失敗しました." << std::endl;
      return false;
    }
  }
...
bool Hardware::create_sync_write_group(const std::string& group_name) {
  ...
  if (joint_groups_[group_name]->sync_write_position_enabled()) {
    ...
    address_indirect_goal_position_[group_name] =
        ADDR_INDIRECT_DATA_29 + total_length;
  ...
  sync_write_groups_[group_name] = std::make_shared<dynamixel::GroupSyncWrite>(
      port_handler_.get(), packet_handler_.get(), ADDR_INDIRECT_DATA_29,
      total_length);
  ...
  for (auto joint_name : joint_groups_[group_name]->joint_names()) {
    ...
    if (!sync_write_groups_[group_name]->addParam(id, init_data)) {
      std::cerr << group_name << ":" << joint_name
                << "のgroupSyncWrite.addParam に失敗しました." << std::endl;
      return false;
    }
  }
...
bool Hardware::set_indirect_address(const std::string& group_name,
  ...
  for (int i = 0; i < len_target; i++) {
    uint16_t target_indirect_address =
        addr_indirect_start + LEN_INDIRECT_ADDRESS * i;
    uint16_t target_data_address = addr_target + i;

    if (!write_word_data_to_group(group_name, target_indirect_address,
                                  target_data_address)) {
      std::cerr << "インダイレクトアドレス:"
                << std::to_string(target_indirect_address);
      std::cerr << "にターゲットデータアドレス:"
                << std::to_string(target_data_address);
...
void Hardware::read_write_thread(
...
bool Hardware::write_byte_data(const uint8_t id, const uint16_t address,
                               const uint8_t write_data) {
  uint8_t dxl_error = 0;
  int dxl_result = packet_handler_->write1ByteTxRx(
      port_handler_.get(), id, address, write_data, &dxl_error);

  if (!parse_dxl_error(std::string(__func__), id, address, dxl_result,
                       dxl_error)) {
    return false;
  }
  return true;
...
bool Hardware::write_byte_data_to_group(const std::string& group_name,
...
bool Hardware::write_word_data(const uint8_t id, const uint16_t address,
  ...
  uint8_t dxl_error = 0;
  int dxl_result = packet_handler_->write2ByteTxRx(
      port_handler_.get(), id, address, write_data, &dxl_error);
...
bool Hardware::write_word_data_to_group(const std::string& group_name,
                                        const uint16_t address,
                                        const uint16_t write_data) {
  if (!joint_groups_contain(group_name)) {
    std::cerr << group_name << "はjoint_groupsに存在しません." << std::endl;
...
bool Hardware::write_double_word_data(const uint8_t id, const uint16_t address,
  ...
  uint8_t dxl_error = 0;
  int dxl_result = packet_handler_->write4ByteTxRx(
      port_handler_.get(), id, address, write_data, &dxl_error);
...
bool Hardware::parse_dxl_error(const std::string& func_name, const uint8_t id,
  ...
  if (dxl_packet_error != 0) {
    ...
    std::cerr << ", PacketError:"
              << std::string(
                     packet_handler_->getRxPacketError(dxl_packet_error))
              << std::endl;
    retval = false;
  }
...
bool Hardware::parse_dxl_error(const std::string& func_name,
                               const int dxl_comm_result) {
  if (dxl_comm_result != COMM_SUCCESS) {
    std::cerr << "Function:" << func_name;
    std::cerr << ", CommError:"
